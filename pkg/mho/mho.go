...
var log = logging.GetLogger("cco-mon", "mho")
...
func NewController(indChan chan *E2NodeIndication, ueStore store.Store, cellStore store.Store, flag bool) *Controller {
...
func (c *Controller) handlePeriodicReport(ctx context.Context, header *e2sm_mho.E2SmMhoIndicationHeaderFormat1, message *e2sm_mho.E2SmMhoIndicationMessageFormat1, e2NodeID string, flag *bool) {
	...
	if ueData == nil {
		...
		c.AttachUe(ctx, ueData, cgi, cgiObject) // return
	}
	...
	log.Infof("[5QI:%v] ue [ID:%v] rsrpServing [cgi:%v] %v, rsrpNeigbors %v \n", ueData.FiveQi, ueData.UeID, cgi, rsrpServing, rsrpNeighbors)
	c.SetUe(ctx, ueData)
...
func (c *Controller) ConvertCgiToTheRightForm(cgi string) string {
	...
	return cgi[0:6] + cgi[14:] + cgi[12:14] + cgi[10:12] + cgi[8:10] + cgi[6:8]
