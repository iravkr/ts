...
import (
	...
	subscriptionutil "github.com/onosproject/cco-mon/pkg/utils/subscription"
...
const (
	...
	// for pci oid: "1.3.6.1.4.1.53148.1.1.2.3"
)
...
func (h *handler) SetPolicyForOcn(ctx context.Context, nodeID string, ocns map[storage.IDs]meastype.QOffsetRange) error { // new function to set a policy for sending the txpower using the E2 RC SM
...
func (h *handler) createSubscription(ctx context.Context, nodeID string, policies []subscriptionutil.PolicyForOcn) error {
	...
	eventTriggerData, err := subscriptionutil.CreateEventTriggerDefinitionUtil()
	if err != nil {
		log.Error(err)
		return err
	}
	...
	action, err := subscriptionutil.CreateSubscriptionActionsUtil(policies)
	if err != nil {
		return err
	}
	...
	subName := fmt.Sprintf("cco-mon-subscription-%v", time.Now().Nanosecond())
	log.Info("cco-mon-subscription-%v", time.Now().Nanosecond())
